...
List allAnagrams(string input_str)
{
    ...
    while(!recurStack.empty()) // while the stack is non-empty
    {
        ...
        for (unsigned int i = 0; i < stacktop.in_str.length(); i++) {
			Pair to_push;
			to_push.fixed_str = stacktop.fixed_str + stacktop.in_str[i]; // add fixed character
			to_push.in_str = stacktop.in_str.substr(0, i) + stacktop.in_str.substr(i+1); // Remaining string
			recurStack.push(to_push); // Push onto the stack
		}
    }
    return final_output;
}

List language(string input, unsigned int k){
	List final_output;
	...
	while (!recurStack.empty()) {
		...
		if (stacktop.fixed_str.length() <= k) {
            ...
            if (stacktop.fixed_str.length() < k) {
                for (char c : stacktop.in_str) {
                    Pair to_push;
                    to_push.fixed_str = stacktop.fixed_str + c;
                    to_push.in_str = stacktop.in_str;
                    recurStack.push(to_push);
                }
            }
...
List stretch(string input, int k) {
	...
	while (!recurStack.empty()) {
		...
		for (int i = 1; i <= k; i++) {
			Pair to_push;
			to_push.fixed_str = stacktop.fixed_str + string(i, stacktop.in_str[0]);
...
List bubbling(string input) {
	...
	while (!recurStack.empty()) {
		...
		Pair no_swap, swap;
		no_swap.fixed_str = stacktop.fixed_str + stacktop.in_str[0];
		no_swap.in_str = stacktop.in_str.substr(1);
		recurStack.push(no_swap);
		if (stacktop.in_str.length() > 1) {
			swap.fixed_str = stacktop.fixed_str + stacktop.in_str[1];
			swap.in_str = stacktop.in_str[0] + stacktop.in_str.substr(2);
			recurStack.push(swap);
		}
	}
	return final_output;
}
...
int main(int argc, char** argv)
{
    ...
    List myList = language(in_str, 2); // initializing the linked list
