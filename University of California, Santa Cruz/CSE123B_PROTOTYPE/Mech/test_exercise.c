    ...
    while (1) {
        ...
        if (err == ESP_OK) {
            ...
            if (root && cJSON_IsArray(root) && cJSON_GetArraySize(root) > 0) {
                ...
                cJSON *jflex = cJSON_GetObjectItem(data, "flexion");
                cJSON *jreps = cJSON_GetObjectItem(data, "reps");
                if (jflex && jreps) {
                    float intensity = (float)jflex->valueint;
                ...
                esp_err_t del_err = esp_http_client_perform(del_client);
                if (del_err == ESP_OK) {
...
void app_main(void) {
    ...
    wifi_init();
    // flex sensor baseline calibration
